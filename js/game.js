...
class Game {
  ...
  bindInput() {
    ...
    c.addEventListener('mousedown', e => { toLogical(e); Audio8.init(); Audio8.resume(); if (e.button === 2) { this.input.rightDown = true; return; } if (e.button !== 0) return;
      ...
      const tr2 = this.transformRect2; if (tr2 && (this.state === 'playing' || this.state === 'wavebreak') && this.input.mouseX >= tr2.x && this.input.mouseX <= tr2.x + tr2.w && this.input.mouseY >= tr2.y && this.input.mouseY <= tr2.y + tr2.h) { if (this.player.char.symbiote) this.player.useCapture(); else this.player.usePull(); return; }
  ...
  drawHUD(ctx) {
    ...
    const w = p.wstate; ctx.fillText(p.venom ? 'VENOM' : p.frog ? 'FROG' : p.beast ? `${p.beastAmmo}/${BEAST_GUN.mag}` : (p.overdrive || p.rushing || (p.driving && !p.car.civil)) ? '∞/∞' : `${w.mag}/${w.reserve === Infinity ? '∞' : w.reserve}`, 30, ay0 + 3);
    ...
    if (p.frog) { ctx.fillStyle = '#9ccf72'; ctx.fillText('TONGUE', 14, this.vh - 27); ctx.fillStyle = '#9aa3b5'; ctx.font = '6px "Press Start 2P", monospace'; ctx.fillText(fit('LMB LASH · SPACE HOP · EAT = HEAL', 116), 14, this.vh - 16); }
    ...
    if (ab && p.form === 'human' && !p.venom && !p.frog && !p.driving) { // hidden whenever the gun itself is put away
