...
| Cry XD | The crying wojak, cut straight from the real meme rather than redrawn: the portrait is the actual face (background removed, 60px) and the in-game sprite is that same face shrunk onto a 16px body with a black outline. Loaded from `assets/img/cry_*.png` over the pixel placeholder once the images arrive. Ability TBD |
