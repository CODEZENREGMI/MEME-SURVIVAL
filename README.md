...
| Doge | The Doge soldier redrawn in pixels: Shiba Inu in "deal with it" pixel shades, camo neck gaiter, tan plate carrier with magazine pouches and a radio on the shoulder. The 60×60 portrait (`assets/img/doge_portrait.png`) is the picture traced to a hand-drawn head/shoulder/antenna outline and snapped to 16 colours, with the shades redrawn pixel by pixel (they don't survive the shrink); the 16×16 sprite is hand-placed in `js/sprites.js`. Balanced stats. **AIRSTRIKE** (`Space`, 25 s recharge): a radio chirp and "BRAVO SIX, GOING IN"; the strike lands on the cursor (clamped to 360 px), marked by a pulsing red ring, crosshairs, red flare smoke and a dashed bomb line while the beeps speed up. 0.9 s later a jet (with its shadow racing across the ground) flies in along your line of sight and drops 7 bombs 26 px apart, 0.09 s between each, zig-zagging 20 px either side of the line (a ~150 px wide strip, shaded on the ground while it's marked): 54 px blasts for 160 damage × his damage multiplier × `enemyHpScale(wave)`, so a hit stays lethal as zombies toughen. (It used to be a single 44 px-wide line after 1.5 s at fixed damage: armed zombies strafing sideways during the countdown simply stepped out of it.) The bombs are friendly (`explode(..., safe)`): they never hurt him or his allies, and he keeps shooting throughout. **MACHINE GUN** (`E`, 25 s / 20 s recharge; his weapon ability moves to `Q` — scroll still swaps weapons — and `F` stays fullscreen): he hauls out a huge minigun (drawn in code: six spinning barrels with rolling shading, brass drum, heat glow, a big muzzle flash) and moves at 85%. Hold LMB for 16-damage **incendiary** rounds every 0.045 s (drawn as a white-hot core in a red-orange flame streak with a glow, shedding embers, sparking where they hit a wall) — every regular zombie hit catches fire and burns for 3 s after the last hit (the shared `burn` damage-over-time); bosses catch for 1.5 s at most once every 4 s so a 25 s burst can't melt them outright — with unlimited ammo and no reloads or weapon swaps, brass casings flying out the side; a 0.35 s spin-up first. The gunfire is a real burst recording cut into a seamless 15-shot loop (`assets/audio/doge_mg_loop.wav`: WAV, because MP3/AAC padding would hiccup at every loop point) that plays for as long as LMB is held and stops dead on release. Second HUD card + `MACHINE GUN` weapon panel |
...
`burn` timer, and it's drawn as real fire by `Zombie.drawFire()`: an orange heat glow on the ground, five flickering flame
tongues in two blended layers (red-orange outside, white-hot core), sparks at the tips, dying down over the last half
second. `burnFx()` runs with every burn tick: the zombie chars progressively darker (`charred`, a dark tint over the
sprite that stays after the fire goes out), sheds embers and rolls off dark smoke (capped when the screen is already full
of particles). The fire is drawn from `draw()`, so the special bosses burn too, and water puts it out.
